# Support

## Documentation

- **[README.md](../README.md)** — overview, use cases, quick start
- **[GUIDE.md](../GUIDE.md)** — installation, configuration, testing, workflows, FAQ
- **[CONTRIBUTING.md](../CONTRIBUTING.md)** — building, testing, adding rules, code standards

## Issues

- [Bug reports](https://github.com/marcuwynu23/git-policy/issues/new?template=bug_report.md)
- [Feature requests](https://github.com/marcuwynu23/git-policy/issues/new?template=feature_request.md)

## Discussions

For questions, ideas, or general help, start a discussion on GitHub.

👉 [github.com/marcuwynu23/git-policy/discussions](https://github.com/marcuwynu23/git-policy/discussions)

## Security

Report security vulnerabilities to the project maintainers privately.
See [SECURITY.md](../SECURITY.md) for details.
